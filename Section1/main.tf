...
resource "azurerm_network_interface" "webinterface01" {
  ...
  depends_on = [ azurerm_subnet.websubnet01 ]
}
...
resource "azurerm_subnet_network_security_group_association" "websubnet01_app_nsg" {
  subnet_id                 = azurerm_subnet.websubnet01.id
  network_security_group_id = azurerm_network_security_group.app_nsg.id
}

resource "azurerm_subnet_network_security_group_association" "appsubnet01_app_nsg" {
...
resource "azurerm_windows_virtual_machine" "webvm01" {
  ...
  depends_on = [ azurerm_network_interface.webinterface01, 
